...
class Uploader {
     ...
     * @return {Array}
     */
    public function upload($files_rename_type = false) {
        return $this->uploadFiles($files_rename_type);
    }
     ...
     * @return bool
     */
    public function call_resize($filename,$width,$height,$destination,$crop,$quality,$rotation)
    {
        return self::resize($filename,$width,$height,$destination,$crop,$quality,$rotation);
    }
     ...
     * @return {null, Array}
     */
    private function uploadFiles($files_rename_type = true) {
        $data = array(
            "hasWarnings" => false,
            "isSuccess" => false,
            "warnings" => array(),
        ...
        if ($this->field['input']) {
            ...
            if ($validate === true) {
                ...
                for($i = 0; $i < count($this->field['input']['name']); $i++) {
                    ...
                    // chunk
                    if ($chunk) {
                        $tmp_name = $uploadDir . '.unconfirmed_' . $this->filterFilename($chunk['name']);
                    ...
                    if($files_rename_type === true)
                    {
                        $metas['file'] = $uploadDir . Carbon::now()->timestamp.rand(0000,9999).'_'.$metas['name'];
                    }
                    else
                    {
                ...
                if (!$data['hasWarnings']) {
                    foreach($data['files'] as $key => $file) {
                        if ($file['chunked'] ? rename($file['tmp_name'], $file['file']) : move_uploaded_file($file['tmp_name'], $file['file'])) {
    ...
    private function getListInputFiles($name = null) {
        ...
        if (isset($_POST[$inputName]) && $this->isJSON($_POST[$inputName])) {
            $values = collect(json_decode($_POST[$inputName],true))->map(function ($value){
                $value['file'] = explode('?v=',$value['file'])[0];
                return $value;
            })->toArray();
            $list = array(
                'list' => array(),
    ...
    private function codeToMessage($code, $file = null) {
        ...
        switch ($code) {
            case UPLOAD_ERR_INI_SIZE:
                $message = "上傳的檔案超出伺服器限制";
                break;
            case UPLOAD_ERR_FORM_SIZE:
                $message = "上傳的檔案超出伺服器限制";
                break;
            case UPLOAD_ERR_PARTIAL:
                $message = "上傳失敗，僅部分檔案上傳，請重試";
                break;
            ...
            case 'accepted_file_types':
                $message = $file['old_name']."格式不符合";
                break;
            case 'file_uploads':
                $message = "服務器不允許上傳檔案類型";
                break;
            case 'max_file_size':
                $message = $file['old_name'] . "檔案過大";
                break;
            case 'max_files_size':
                $message = "檔案過大";
                break;
    ...
    /**
     * other class can use mime_content_type
     *
     * @param $file
     * @return mixed|string
     */
    public function call_mime_content_type($file)
    {
        return self::mime_content_type($file);
    }
